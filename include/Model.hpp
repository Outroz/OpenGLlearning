...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        if (mesh->mMaterialIndex >= 0)
        {
            ...
            vector<Texture> diffuseMaps = loadMaterialTextures(mat, aiTextureType_DIFFUSE, "textrue_diffuse");
            texture.insert(texture.end(), diffuseMaps.begin(), diffuseMaps.end());
            vector<Texture> specualrMaps = loadMaterialTextures(mat, aiTextureType_SPECULAR, "texture_specular");
            texture.insert(texture.end(), specualrMaps.begin(), specualrMaps.end());
            ...
            texture.insert(texture.end(), roughnessMaps.begin(), roughnessMaps.end());
            
        }

        return Mesh(vertices, texture, indices);
    }
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            if (!skip)
            {
                ...
                std::cout << mat->GetTextureCount(type) << typeName << str.C_Str() << std::endl;
